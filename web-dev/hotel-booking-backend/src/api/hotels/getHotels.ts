...
const querySchema = z.object({
  city: z.string().optional().transform((val) => val?.toLowerCase()),
  country: z.string().optional().transform((val) => val?.toLowerCase()),
...
export async function handleGetHotels(req: AuthenticatedRequest, res: Response) {
  ...
  if (params.minPrice !== undefined) {
    ...
    hotels = hotels.filter((hotel) => hotel.rooms.some((room) => room.price_per_night > minPrice))
  }

  if (params.maxPrice !== undefined) {
    const maxPrice = params.maxPrice
    hotels = hotels.filter((hotel) => hotel.rooms.some((room) => room.price_per_night < maxPrice))
  }
